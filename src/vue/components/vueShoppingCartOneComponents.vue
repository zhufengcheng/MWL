<template>
    <div class="vueShoppingCartOneComponents">
...
<script>
    export default {
        name: 'vueShoppingCartOneComponents',
        components: {},
        created() {
            this.$CRJ.$on('addCart', goodsOne => {
                const ret = this.cart.find(v => v.id === goodsOne.id)
                if (ret) {
                    ret.count += 1;
                } else {
                    this.cart.push({
